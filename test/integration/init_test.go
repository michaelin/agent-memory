...
var _ = Describe("agent-memory instructions", func() {
	It("outputs agent blurb to stdout", func() {
		...
		Expect(exitCode).To(Equal(0))
		Expect(stdout).To(ContainSubstring("# Agent Memory"))
		Expect(stdout).To(ContainSubstring("write-note"))
		Expect(stdout).To(ContainSubstring("--json"))
		Expect(stdout).To(ContainSubstring("promote"))
		Expect(stdout).To(ContainSubstring("deprecate"))
		Expect(stdout).To(ContainSubstring("Librarian"))
		Expect(stdout).To(ContainSubstring("confirmed"))
		Expect(stdout).To(ContainSubstring("templates"))
		Expect(stdout).To(ContainSubstring("At the end of every task"))
		Expect(stdout).To(ContainSubstring("epistemic types"))
		Expect(stdout).To(ContainSubstring("observation"))
		Expect(stdout).To(ContainSubstring("constraint"))
		Expect(stdout).To(ContainSubstring("assumption"))
	})
})
